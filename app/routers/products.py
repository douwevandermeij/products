...
from app.routers.utils import TokenPayload, get_payload
...
@router.post(Routes.PRODUCTS, response_model=Product, status_code=status.HTTP_201_CREATED)
def add_product(
    product: ProductContract, payload: TokenPayload = Depends(get_payload)
):
...
def products(payload: TokenPayload = Depends(get_payload)):
    return list(
...
def product(product_id: UUID, payload: TokenPayload = Depends(get_payload)):
...
def delete_product(product_id: UUID, payload: TokenPayload = Depends(get_payload)):
